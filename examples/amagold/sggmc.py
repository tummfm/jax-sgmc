...
mcmc = npy_inf.MCMC(kernel, num_warmup=1000, num_samples=1000)
...
# Adaption strategy
mass_adaption = adaption.mass_matrix(burn_in=2500, diagonal=False)
...
default_integrator = integrator.obabo(potential_fn,
                                      ...
                                      steps=10,
                                      friction=100,
                                      const_mass={"w": 0.1 * jnp.ones(4), "sigma": 0.001})
...
sggmc = solver.sggmc(default_integrator, full_potential_fn)

full_data_state = full_data_init()
amagold_state = sggmc[0](sample, full_data_state)
