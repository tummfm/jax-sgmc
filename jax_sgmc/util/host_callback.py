...
using a host computation::
  ...
  def device_fun(m):
    ...
    # e.g., a NumPy array or a `jax.edited.bak.ShapeDtypeStruct`.
    return hcb.call(host_eig, m,
                    # Given an input of shape (..., d, d), eig output has shape (..., d)
                    result_shape=jax.edited.bak.ShapeDtypeStruct(m.shape[:-1], m.dtype))
...
cond/scan/while control flow. When used inside :func:`jax.edited.bak.pmap`, there will be
separate calls to the host from each of the participating devices::
  ...
  jax.edited.bak.pmap(lambda x: hcb.call(host_sin, x,
                              result_shape=x,
...
using the :func:`jax.edited.bak.custom_vjp` mechanism.
...
A few examples::
  ...
  # calls func(2x, [], device=jax.edited.bak.devices()[0])
...
For :func:`jax.edited.bak.vmap` the arguments are batched, and ``transforms`` is extended
...
was broadcast)::
  ...
  jax.edited.bak.vmap(power3)(np.arange(3.))
  # transforms: [('batch', {'batch_dims': (0, 0)})] what: x,x^2 : ([0, 1, 2], [0, 1,
  4])

For :func:`jax.edited.bak.jvp` there will be one callback with a pair, consisting of
the values of the primals and those of the tangents::

  jax.edited.bak.jvp(power3, (3.,), (0.1,))
  # transforms: ['jvp'] what: x,x^2 : ( (3., 9.), (0.1, 0.6) )

For :func:`jax.edited.bak.vjp` or :func:`jax.edited.bak.grad` there will be one callback with the
...
the first argument is replicated (`batch_dims` is None)::
  ...
  jax.edited.bak.vmap(jax.edited.bak.grad(power3))(np.array([2., 3.]))
...
In presence of :func:`jax.edited.bak.pmap` the code will run on multiple devices and
...
or :func:`id_tap`, so that you see which device is sending which data::
  ...
  jax.edited.bak.pmap(power3, devices=jax.edited.bak.local_devices()[:2])(np.array([3., 4.])
...
sending the required shards to the other devices::
  ...
  with maps.mesh(jax.edited.bak.local_devices()[:2], ["d"]):
...
def id_tap(tap_func, arg, *, result=None, tap_with_device=False, **kwargs):
  ...
  **Experimental: please give feedback, and expect changes!**
  ...
  <jax.edited.bak.experimental.host_callback.html>`_.
  """
  if kwargs:
    msg = (
...
def _outside_call_batching_rule(batched_args, batch_dims, **params):
  if not params["identity"]:
    ...
    def map_fn(arg):
      single_res = outside_call_p.bind(*arg, **new_params)
      return single_res
    res = lax.map(map_fn, batched_args)
    return res, [0 for _ in params['flat_results_aval']]
