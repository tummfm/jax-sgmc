...
default_random_thinning = scheduler.random_thinning(
  ...
  selections=200)
temperature = scheduler.constant_temperature(tau=1.0)

default_scheduler = scheduler.init_scheduler(temperature=temperature,
                                             step_size=default_step_size,
                                             burn_in=burn_in,
                                             thinning=default_random_thinning)

default_sgld = solver.parallel_tempering(default_integrator)

default_run = solver.mcmc(default_sgld, default_scheduler)

init_state = default_sgld[0](sample, sample_2)
print("Init state")
print(init_state)

default_results = default_run(init_state, schedulers=[{'temperature': {'tau': 100}}], iterations=iterations)["samples"]
